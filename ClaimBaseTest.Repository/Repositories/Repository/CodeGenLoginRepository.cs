...
{
    public class CodeGenLoginRepository : WizTestRepositoryBase, ICodeGenLoginRepository
    {
        public CodeGenLoginRepository(IWizTestConnectionFactory TConnectionFactory) : base(TConnectionFactory)
        {
        ...
        public async Task<int> ChangePwd(CodeGenLogin Upd)
		{
			try
			{
				...
				var results = await Connection.ExecuteAsync("USP_CPKG_1_USERELOGIN1", p, commandType: CommandType.StoredProcedure);
				return results;
			}
			catch (Exception ex)
			{
